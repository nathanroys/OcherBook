...
int UnzipCache::unzipFile(const char* pattern, std::string* matchedName)
{
    ...
    if (tfile) {
        ...
        if (err != UNZ_OK) {
            m_log->error("unzOpenCurrentFilePassword: %d", err);
        } else {

            do {
                err = unzReadCurrentFile(m_uf, buf.data(), buf.size());
                if (err < 0) {
                    m_log->error("unzReadCurrentFile: %d", err);
                }
            } while (err > 0);
        }
        tfile->data.assign(buf.data(), buf.size());

        if (err == UNZ_OK) {
            err = unzCloseCurrentFile(m_uf);
            if (err != UNZ_OK) {
                m_log->error("unzCloseCurrentFile: %d", err);
            }
        } else
...
int UnzipCache::unzip(const char* pattern, std::list<std::string>* matchedNames)
{
    ...
    if (err != UNZ_OK) {
        m_log->error("unzGetGlobalInfo: %d", err);
        return -1;
    }
    ...
    for (i = 0; i < gi.number_entry; i++) {
        ...
        if ((i + 1) < gi.number_entry) {
            ...
            if (err != UNZ_OK) {
                m_log->error("unzGoToNextFile: %d", err);
                return -1;
            }
        }
    }
